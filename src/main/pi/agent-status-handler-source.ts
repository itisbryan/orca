...
export function getPiAgentStatusHandlerSourceLines(): string[] {
  return [
    ...
    '// Why: Pi runs subagents as nested child processes that inherit the lead',
    "// pane's env (including ORCA_PANE_KEY), each loading this extension. The",
    '// first (lead) process to load claims status reporting for the pane and',
    '// marks the env; any Pi it spawns inherits the marker and stays silent, so',
    '// a subagent agent_end never fires a false "finished" notification on the',
    '// lead pane. Keyed on nesting (not hasUI) so a top-level non-interactive',
    '// `pi -p` run still reports its own completion.',
    'export default function (pi): void {',
    "  const isNestedAgent = process.env.ORCA_PI_STATUS_OWNED === '1'",
    "  process.env.ORCA_PI_STATUS_OWNED = '1'",
